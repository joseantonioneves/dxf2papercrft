  ...
  */
  ...
  for(int offset = 3; offset < 4; offset++) {
	...
	if(faceArray[i].drawn == false) {
	  ...
	  if(faceArray[i].generateCutoutSheet(DXFFile, last_offset, last_dir, 0, 
  ...
  do {
    ...
    // search for not yet drawn face
    
    for(last_face = 0; last_face < initial_facecnt; last_face++) {
      if(faceArray[last_face].drawn == false) {
	...
	if(faceArray[last_face].writeToDXF(DXFFile, last_offset, last_dir, 0, normal_vec, initial_facecnt, ALLOW_INTERSECTION) == false) {
	  cout << "   failed writing inital face" << endl;
	  faceArray[last_face].processed = false;
	} // if
	else {
	  gen_face_index = 0;
	  gen_face_array[gen_face_index++] = last_face;
	  cout << "wrote initial face #" << counter << endl;
	  counter++;
    ...
    if(last_face == initial_facecnt) {
      cout << "error: could not find another face." << endl;
      break;
    } // if
    
    do {
      int start_with_index_old_face; // we need this to unset connected_vertex in case drawing fails
      
      current_index = getNextFace(last_face, initial_facecnt, last_offset, last_dir, start_with_index, 
				  start_with_index_old_face, normal_vec);
      
      if((current_index != -1) && (CONNECT_FACES == true)) {
	
	cout << "write next face #" << current_index << endl;
	
	if(faceArray[current_index].writeToDXF(DXFFile, last_offset, last_dir, start_with_index, normal_vec, 
					       initial_facecnt, ALLOW_INTERSECTION) == false) {
	  cout << "   failed" << endl;
	  
	  // DEBUG: DID WE DRAW TWO FACES
	  //        ONTO ONE ANOTHER??
	  
	  int my_counter = 0;
	  
	  for(int i = 0; i < faceArray[current_index].no_points; i++) {
	    if(fabs((faceArray[last_face].projection[i] - faceArray[current_index].projection[i]).Norm2()) < epsilon) my_counter++;
	  } // for
	  
	  if(my_counter == faceArray[current_index].no_points) {
	    cout << "     ALARM: 2 faces projected onto one another" << endl;
	    //	    exit(-1);
	  } // if
	  else {
	    cout << "   number of overlapping points: " << my_counter << endl;
	  } // else
	  
	    // create the adherents for the faces written so far
	  
	  faceArray[last_face].adherent_connected[start_with_index_old_face] = false;
	  
	  for(int i = 0; i < gen_face_index; i++) {
	    faceArray[gen_face_array[i]].writeAdherent(DXFFile, extend3D, facecnt);
	  } // for
	  gen_face_index = 0;
	  
	} // if
	else {
	  gen_face_array[gen_face_index++] = current_index;
	  last_face = current_index;
	  counter++;
	} // else
	...
	if(current_index != -1)	 
	  faceArray[current_index].adherent_connected[start_with_index] = false;
    ...
    } while((current_index != -1) && (CONNECT_FACES == true));
    
  } while(counter < initial_facecnt);
  
  // print remaining adherents for gluing the model together
